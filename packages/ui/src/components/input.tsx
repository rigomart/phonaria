...
function Input({ className, size = "default", unstyled = false, ...props }: InputProps) {
	// Determine the type of the input for class application
	const typeKey = props.type === "search" ? "search" : props.type === "file" ? "file" : "default";
	const isUnstyled = !!unstyled;

	return (
		<span
			className={
				isUnstyled ? cn(className) : cn(inputControlVariants({ unstyled: isUnstyled }), className)
			}
			data-size={size}
