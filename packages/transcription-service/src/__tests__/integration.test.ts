...
const INTEGRATION_TEST_ENABLED =
	process.env.INTEGRATION_TEST_TURSO_URL &&
	process.env.INTEGRATION_TEST_TURSO_TOKEN;

describe.skipIf(!INTEGRATION_TEST_ENABLED)(
	"Turso integration (read-only)",
	() => {
		let dbClient: ReturnType<typeof createDbClient>;

		beforeAll(() => {
			dbClient = createDbClient({
				url: process.env.INTEGRATION_TEST_TURSO_URL!,
