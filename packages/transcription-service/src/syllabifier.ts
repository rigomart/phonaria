...
/**
 * Convert a sequence of CMU tokens into syllables.
 * Uses the maximal onset principle for syllable boundary determination.
 */
export function syllabify(cmuTokens: string[]): TranscriptionSyllable[] {
	...
	if (nucleiIndices.length === 0) {
		return [
			{
				phonemes: mapToTranscriptionPhonemes(phonemes),
				stress: "none",
			},
		];
	}

	const syllables: TranscriptionSyllable[] = [];
	let currentStart = 0;
	...
	for (let i = 0; i < nucleiIndices.length; i++) {
		...
		syllables.push({
			phonemes: mapToTranscriptionPhonemes(syllablePhonemes),
