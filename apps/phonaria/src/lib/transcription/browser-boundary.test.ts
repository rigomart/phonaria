...
const clientFiles = [
	...
	"src/lib/transcription/spelling-context.ts",
...
const forbidden = [
	...
	/@\/server\/definition/,
	/@\/server\/spelling-context/,
	/@\/lib\/transcription\/spelling-context-service/,
	/@\/lib\/jev\//,
	/openrouter\.ai/,
	/OPENROUTER_API_KEY/,
...
describe("browser transcription boundary", () => {
	...
	it("keeps curated-10k behind a client-only dynamic import", () => {
		...
		const serverFiles = [
			...
			"src/server/spelling-context.ts",
			"src/server/choose-spelling.ts",
			"src/lib/transcription/spelling-context-service.ts",
