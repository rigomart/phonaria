...
import { vowels, phonixUtils } from "shared-data";
    ...
    <>
      <div className="mt-4 w-full overflow-x-auto">
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr>
              <th className="sticky left-0 z-10 bg-background p-2 text-left">Height \\ Frontness</th>
              {FRONTS.map((f) => (
            ...
            <div className="space-y-4">
              <DialogHeader>
                <DialogTitle className="text-2xl font-semibold">
                  {selected.symbol} <span className="ml-2 text-base text-muted-foreground">{toPhonemic(selected.symbol)}</span>
                </DialogTitle>
              ...
              <section className="text-sm">
                ...
                <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
                  <li>Height: <span className="capitalize text-foreground">{selected.articulation.height}</span></li>
                  <li>Frontness: <span className="capitalize text-foreground">{selected.articulation.frontness}</span></li>
                  <li>Lip rounding: <span className="capitalize text-foreground">{selected.articulation.roundness}</span></li>
                  <li>Tenseness: <span className="capitalize text-foreground">{selected.articulation.tenseness}</span></li>
                  {selected.articulation.rhoticity ? (
                    <li>Rhoticity: <span className="capitalize text-foreground">{selected.articulation.rhoticity}</span></li>
                  ) : null}
                </ul>
              </section>
              ...
              <section className="text-sm">
                ...
                <ul className="space-y-2">
                  {selected.examples.map((ex) => (
                    <li key={ex.word} className="flex items-center justify-between gap-3 rounded-md border p-2">
                      <div>
