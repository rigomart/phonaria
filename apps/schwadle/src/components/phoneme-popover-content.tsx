...
export function PhonemePopoverContent({ phonemeId }: PhonemePopoverProps) {
	...
	return (
		<div className="flex w-72 flex-col gap-2">
			...
			{!isDiphthong && <p className="text-xs leading-snug text-muted-foreground">{label}</p>}
...
function ArticulationDiagram({
	...
	if (articulation.category === "consonant") {
		...
		return (
			<div className="relative aspect-square w-full overflow-hidden rounded-md bg-neutral-900/60">
				<img
		...
		<div className="flex flex-col gap-2">
			<div className="relative aspect-square w-full overflow-hidden rounded-md bg-neutral-900/60">
				<img
					...
					alt={`${label} (${stopLabel})`}
					className={cn(
						"absolute inset-0 h-full w-full object-cover transition-opacity duration-200 ease-out motion-reduce:transition-none",
...
function GlidePositionButton({
	...
	return (
		<button
			...
			onClick={onClick}
			className={cn(
				"group flex flex-1 cursor-pointer items-center gap-1.5 rounded-lg border px-2.5 py-2 text-left transition-all duration-150",
				active
		...
		>
			...
			<span className="text-[10px] leading-tight text-muted-foreground">{roundness}</span>
			<ChevronDown
				className={cn(
			...
			{BACKNESS_LABELS.map(({ value, label }, i) => (
				<g key={value}>
					...
					<text
						...
						y={topLabelY}
						className="fill-muted-foreground text-[5px] uppercase"
						fontWeight={600}
